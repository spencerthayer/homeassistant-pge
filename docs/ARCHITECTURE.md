...
| `tod_schedule.py` / `tod_pricing.py` | Offline E-TOU schedule engine (3 periods, weekday Pacific windows, observed holidays) + rate-card resolution (override → portal snapshot → defaults) and transition coordinator |
| `coordinator.py` | Polling from options, correction window, import lock, billing hook, TOD transition watchdog |
...
| `frontend/data.js` + `charts.js` + `pge-panel.js` + `theme.js` | Buildless ES-module custom panel (Apache ECharts). Fetch path: recorder stats → `projectDirectionalUsage` / `projectUsageSeries` → Usage hero + Range accounting. Generating accounts render one signed **Grid flow** bar series (import above zero / export below) and **Net interval amount** when compensation is observed; import-only accounts keep positive bars + import cost. Primary range buttons `24h` / This cycle / Last cycle / 7 days / Month plus More…; bill-bound ranges use statement dates; Range accounting / rollup `<details>` persist open/closed; Billing always expanded; ranges end at Pacific midnight exclusive. **At a glance** KPI tiles copy label/value/delta to the clipboard on click (Enter/Space). Programs grid includes EV Smart Charging / Smart Battery enrollment with Eligible/Enrolled/Not enrolled. **Time of Day** section: period/rate KPIs, week grid, usage-by-period (imported cost + TOD-priced column), local estimate hero (TOD-priced kWh vs billed imported energy for a `#tod` range picker, default Last cycle; inferred ¢/kWh; collapsed rate-card TOD vs Basic model), official portal rate-compare card when present. Theme tokens from `theme.js`. |
