...
- Token renew DNS/TLS blips (`PGEConnectionError` during Cognito/Apigee login) soft-fail the same way — they must not raise `UpdateFailed` and blank cold-start sensors. Tip intervals, account/programs/tracker snapshots, and freshness timestamps are persisted on the import Store and rehydrated in `async_load_store` so a failed **first** poll after HA restart keeps yesterday/billing/program entities at last-known values (not `unknown`). Recorder-backed lifetime totals still refresh from statistics; Store checkpoints alone are not enough for tip/billing sensors.
...
5. Scheduled poll (default every 4 hours from midnight Pacific) re-fetches the correction window **even while history backfill is running**. Fetch is lock-free; `import_lock` only serializes recorder writes. If yesterday’s hourly is still incomplete, import any hours returned, demote the day from `completed`, and catch up every 2 hours until it validates. Skipping the poll during backfill froze `latest_available_interval` for hours.
6. Auto-backfill on setup/reload when `auto_backfill` and history is incomplete. Hourly walks **newest incomplete local days first**. When the backfill job exits, request an immediate correction-window refresh so the tip does not wait for the next 4h grid slot.
...
8. **Hang recovery:** long-lived jobs use `hass.async_create_background_task` (never `hass.async_block_till_done` from import paths — that deadlocks when a tracked backfill waits on `import_lock` held by a poll). Progress heartbeat + stall watchdog (default 30m), hard-release after cancel grace, generation-guarded state so orphans cannot clobber a newer job, per-tier `asyncio.wait_for` (2h), and I/O-resilient `.storage` saves: non-critical saves **coalesce** into one debounced background write (3s window) so backfill bursts produce one disk write instead of dozens; every save is content-hash **deduped** so an unchanged payload skips the disk entirely (and no longer bumps `last_commit`, which previously forced full rewrites); the wall-clock save timeout is 60s for slow storage; checkpoint durability is explicit — a pre-import `dirty_from` marker must land before any recorder mutation or the import is gated: backfill batches write it inline with one retry (`_CHECKPOINT_SAVE_ATTEMPTS = 2`) and **defer the whole batch** on timeout (hourly/daily/monthly leave those days incomplete without marking them failed), while correction-window polls soft-fail the cycle and failed-range retries skip the day on a single timed-out attempt, and the marker transaction (set → durable save → recorder import → clear) holds `import_lock` end-to-end in all three paths so concurrent importers cannot interleave markers. Critical saves snapshot call-time state before any await; post-import clears are durable as well (correction saves synchronously with up to two attempts), while backfill post-import completion and tier/progress saves enqueue through the debounce writer where losing one only narrows recovery coverage until the next durable checkpoint absorbs it (`backfill._async_save_checkpoint(durable=…)`, `coordinator._async_save_import_store_resilient`). Critical checkpoints stay synchronous (`dirty_from` lands before each recorder import), terminal sync states persist durably rather than debounced, and unload/clear flush or cancel any pending writer. A timed-out save keeps running executor-side behind `asyncio.shield` and stays tracked per entry (`_SaveState.inflight`): the reconcile → dedupe → write sequence is serialized by a per-entry lock, later writes — including the empty clear write — await a parked write before writing newer payload, reconciling one (running or completed) invalidates the dedupe hash so reverted payloads force a corrective rewrite, and loads wait out a parked write so a prompt reload cannot hydrate stale data (a write still pending past the window fails setup into `ConfigEntryNotReady` retry instead); unload keeps the coordination registered until the parked write drains (released only if unclaimed) so a prompt reload cannot overlap it on a fresh Store. Soft-fail poll paths contain the terminal-write timeout and keep last-known data. Boot rewrites a restored `backfilling` status to `failed` until a real resume task starts.
...
| `tod_schedule.py` / `tod_pricing.py` | Offline E-TOU schedule engine (3 periods, weekday Pacific windows, observed holidays) + rate-card resolution (override → catalog → portal snapshot → defaults) and transition coordinator |
| `tod_tariff.py` / `tariff_sources.py` / `tariff_store.py` / `tariff_updater.py` | On-device tariff catalog: effective-dated TOD + Basic rates auto-discovered from PGE public Gatsby page-data and PDF sources, with bundled seed rates, supersession audit trail, and domain-global coordinator |
| `coordinator.py` | Polling from options, correction window, import lock, billing hook, TOD transition watchdog, tariff updater lifecycle |
...
| `store.py` | Versioned per-entry import state (+ billing ledger checkpoint); debounced/coalesced writer with content-hash dedupe, `async_flush_import_state`, and timeout-resilient critical saves |
...
| `frontend/data.js` + `charts.js` + `pge-panel.js` + `theme.js` | Buildless ES-module custom panel (Apache ECharts). Fetch path: recorder stats → `projectDirectionalUsage` / `projectUsageSeries` → Usage hero + Range accounting. Generating accounts render one signed **Grid flow** bar series (import above zero / export below) and **Net interval amount** when compensation is observed; import-only accounts keep positive bars + import cost. Primary range buttons `24h` / This cycle / Last cycle / 7 days / Month plus More…; bill-bound ranges use statement dates; Range accounting / rollup `<details>` persist open/closed; Billing always expanded; ranges end at Pacific midnight exclusive. **At a glance** KPI tiles copy label/value/delta to the clipboard on click (Enter/Space). Programs grid includes EV Smart Charging / Smart Battery enrollment with Eligible/Enrolled/Not enrolled. **Time of Day** section: period/rate KPIs, week grid, usage-by-period, dual-source local estimate hero (catalog-based TOD vs Basic with official reconciliation, flat-rate detection), tariff status block, official portal rate-compare card when present. Theme tokens from `theme.js`. |
