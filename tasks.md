...
- [x] README redesign via `beautify-github-readme` skill (docs-only, no version bump): new `assets/readme/` hero + workflow SVGs (project palette: kWh blue / cost orange / night navy), real `/pge` panel proof screenshots (At a glance, Usage, Analytics, Billing) with numeric values OCR-redacted (Swift Vision redactor), reordered to value → proof → mechanism → install, removed README mermaid duplicates (already in ARCHITECTURE.md), moved logger-filter + sqlite troubleshooting to new `docs/HA_TROUBLESHOOTING.md`
